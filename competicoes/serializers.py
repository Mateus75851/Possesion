    ...
    def validate(self, data):
        ...
        if vitorias > gols_feitos:
            raise serializers.ValidationError(
                {
                    'vitorias': 'É matematicamente impossível se ter mais vitórias do que gols feitos'
                }
            )
...
class EstatisticaSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if chutes_a_gol > chutes:
            raise serializers.ValidationError(
                {
                    'chutes_a_gol': 'Não pode haver mais chutes a gol do que chutes totais'
                }
            )
...
class PartidaSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        estatisticas_mandante = data.get(
            'estatisticas_mandante',
            instance.estatisticas_mandante if instance else None,
        )
        estatisticas_visitante = data.get(
            'estatisticas_visitante',
            instance.estatisticas_visitante if instance else None,
        )
        ...
        if estatisticas_mandante and not estatisticas_visitante:
            raise serializers.ValidationError(
                {
                    'estatisticas_mandante': 'Não se pode ter as estatísticas do mandante sem as do visitante'
                }
            )
        ...
        if mandante == visitante:
            raise serializers.ValidationError(
                {'mandante': 'Um clube não pode jogar contra ele mesmo'}
            )
        ...
        if estatisticas_mandante:
            porcentagem_posse_de_bola_mandante = (
                estatisticas_mandante.get('porcentagem_posse_de_bola')
                if type(estatisticas_mandante) == dict
                else estatisticas_mandante.porcentagem_posse_de_bola
            )
            porcentagem_posse_de_bola_visitante = (
                estatisticas_visitante.get('porcentagem_posse_de_bola')
                if type(estatisticas_visitante) == dict
                else estatisticas_visitante.porcentagem_posse_de_bola
            )

            if (
                porcentagem_posse_de_bola_mandante + porcentagem_posse_de_bola_visitante
                != 100
            ):
                raise serializers.ValidationError(
                    {
                        'estatisticas_mandante': 'As porcentagens de posse de bola do mandante e do visitante precisam somar 100'
                    }
                )

        if escalacao_mandante:
            atletas_invalidos_mandante = [
                escalacao_slot['atleta'].nome
                for escalacao_slot in escalacao_mandante
                if escalacao_slot['atleta'].clube != mandante.clube
            ]

            if atletas_invalidos_mandante:
                raise serializers.ValidationError(
                    {
                        'escalacao_mandante': f'Os seguintes atletas não pertencem ao clube mandante: {atletas_invalidos_mandante}'
                    }
                )

        if escalacao_visitante:
            atletas_invalidos_visitante = [
                escalacao_slot['atleta'].nome
                for escalacao_slot in escalacao_visitante
                if escalacao_slot['atleta'].clube != visitante.clube
            ]

            if atletas_invalidos_visitante:
                raise serializers.ValidationError(
                    {
                        'escalacao_visitante': f'Os seguintes atletas não pertencem ao clube visitante: {atletas_invalidos_visitante}'
                    }
                )
            ...
            for rodada_anterior in range(1, rodada):
                ...
                for partida in partidas:
                    if partida.status == 'P':
                        raise serializers.ValidationError(
                            {
                                'rodada': f'A partida {partida.__str__()} ainda está pendente'
                            }
                        )
    ...
    def update(self, instance, validated_data):
        ...
        estatisticas_visitante = validated_data.pop(
            'estatisticas_visitante', instance.estatisticas_visitante
        )
        ...
        if estatisticas_mandante_vieram or estatisticas_visitante_vieram:
            with transaction.atomic():
                estatisticas_mandante = (
                    estatisticas_mandante
                    if type(estatisticas_mandante) == dict
                    else EstatisticaSerializer(estatisticas_mandante).data
                )
                estatisticas_visitante = (
                    estatisticas_visitante
                    if type(estatisticas_visitante) == dict
                    else EstatisticaSerializer(estatisticas_visitante).data
                )

                # mexe nas instâncias de ESTATISTICA
                estatisticas_mandante, _ = Estatistica.objects.update_or_create(
                    id=estatisticas_mandante_antigas.id
                    if estatisticas_mandante_antigas
                    else None,
                    defaults=estatisticas_mandante,
                )

                estatisticas_visitante, _ = Estatistica.objects.update_or_create(
                    id=estatisticas_visitante_antigas.id
                    if estatisticas_visitante_antigas
                    else None,
                    defaults=estatisticas_visitante,
                )
    ...
    def to_representation(self, instance):
        representacao = super().to_representation(
            instance
        )  # retorna um dicionário de primitivos
        representacao['campeonato'] = instance.campeonato.__str__()
        representacao['mandante'] = instance.mandante.clube.nome
        representacao['visitante'] = instance.visitante.clube.nome

        escalacao_mandante_queryset = instance.escalacao_slots.filter(
            atleta__clube=instance.mandante.clube
        )
        escalacao_visitante_queryset = instance.escalacao_slots.filter(
            atleta__clube=instance.visitante.clube
        )

        escalacao_mandante_lista_dicionarios = EscalacaoSlotSerializer(
            escalacao_mandante_queryset, many=True
        ).data
        escalacao_visitante_lista_dicionarios = EscalacaoSlotSerializer(
            escalacao_visitante_queryset, many=True
        ).data
...
class GolSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            atleta.clube != partida.mandante.clube
            and atleta.clube != partida.visitante.clube
        ):
            raise serializers.ValidationError(
                {
                    'atleta': 'O clube a qual esse atleta é relacionado não participa dessa partida'
                }
            )
...
class CadastroClubesSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if quantidade_clubes_recebida != quantidade_clubes_necessaria:
            raise serializers.ValidationError(
                {
                    'clubes': f'{quantidade_clubes_necessaria} clubes são necessários, mas {quantidade_clubes_recebida} foram recebidos'
                }
            )
        return data
